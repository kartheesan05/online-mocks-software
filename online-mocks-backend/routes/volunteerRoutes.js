...
// Get allocated HRs
router.get('/allocated-hrs', auth, async (req, res) => {
  try {
    const hrs = await HR.find({
      allocatedVolunteers: req.volunteer.id
    });
    res.json(hrs);
  } catch (error) {
...
router.get('/students/:hrId', auth, async (req, res) => {
  try {
...
router.post('/deallocate-student/:studentId/:hrId', auth, async (req, res) => {
  try {
